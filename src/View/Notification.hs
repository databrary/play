...
import {-# SOURCE #-} Controller.Volume
import {-# SOURCE #-} Controller.Slot
import {-# SOURCE #-} Controller.AssetSegment
import View.Authorize (authorizeSiteTitle)
import View.Party (htmlPartyViewLink)
import View.Volume (htmlVolumeViewLink)
import View.VolumeAccess (volumeAccessTitle, volumeAccessPresetTitle)
import View.Container (releaseTitle)
import View.Html
...
mailNotification msg Notification{..} = case notificationNotice of
  ...
  NoticeSharedVolume ->
    agent <> " shared the following volume, " <> volume <> ", on  To review, go to: "
    <> mailLink viewVolume (HTML, maybe noId volumeId notificationVolume) []
  NoticeNewsletter ->
